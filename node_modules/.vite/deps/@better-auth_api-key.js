import { d as isDevelopment, p as isTest } from "./env-DefybXhY.js";
import { t as APIError$2 } from "./error-CHLhyaje.js";
import "./error-BxR608yR.js";
import { Bn as record, Dt as _enum, Mn as object, Nt as any, Pt as array, Rt as boolean, Wn as string, i as string$1, jn as number, r as number$1 } from "./external-BZPJ4xeu.js";
import "./context-CaDZRUMQ.js";
import "./dist-DCo_agu_.js";
...
import { createAuthEndpoint as createAuthEndpoint$1, createAuthMiddleware as createAuthMiddleware$1 } from "./@better-auth_core_api.js";
import { generateId as generateId$1 } from "./@better-auth_core_utils_id.js";
import { safeJSONParse } from "./@better-auth_core_utils_json.js";
import { t as API_KEY_ERROR_CODES } from "./error-codes-vwTWW2ez-BK9mdKUs.js";
import { BetterAuthError } from "better-auth";
import { APIError as APIError$1, getSessionFromCtx, sessionMiddleware } from "better-auth/api";
...
async function checkOrgApiKeyPermission(ctx, userId, organizationId, requiredAction) {
	...
	if (!orgOptions) {
		...
		throw APIError$2.from("INTERNAL_SERVER_ERROR", msg);
	}
	...
	if (!member) {
		...
		throw APIError$2.from("FORBIDDEN", msg);
	}
...
function isAPIError(error) {
	return error instanceof APIError$1 || error instanceof APIError$2 || error?.name === "APIError";
}
...
function createApiKey({ defaultKeyGenerator, configurations, schema, deleteAllExpiredApiKeys }) {
	return createAuthEndpoint$1("/api-key/create", {
		method: "POST",
		...
		if (isClientRequest && (refillAmount !== void 0 || refillInterval !== void 0 || rateLimitMax !== void 0 || rateLimitTimeWindow !== void 0 || rateLimitEnabled !== void 0 || permissions !== void 0 || remaining !== null)) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.SERVER_ONLY_PROPERTY);
		if (ctx.request && ctx.body.userId !== void 0) throw APIError$2.from("UNAUTHORIZED", API_KEY_ERROR_CODES.UNAUTHORIZED_SESSION);
		...
		if (referencesType === "organization") {
			...
			if (!orgId) {
				...
				throw APIError$2.from("BAD_REQUEST", msg);
			}
			const userId = session?.user.id || ctx.body.userId;
			if (!userId) throw APIError$2.from("UNAUTHORIZED", API_KEY_ERROR_CODES.UNAUTHORIZED_SESSION);
			...
			if (!session?.user.id) {
				...
				throw APIError$2.from("UNAUTHORIZED", msg);
			}
		...
		if (metadata) {
			if (opts.enableMetadata === false) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.METADATA_DISABLED);
			if (typeof metadata !== "object") throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.INVALID_METADATA_TYPE);
		}
		...
		if (prefix) {
			if (prefix.length < opts.minimumPrefixLength) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.INVALID_PREFIX_LENGTH);
			if (prefix.length > opts.maximumPrefixLength) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.INVALID_PREFIX_LENGTH);
		}
		if (name) {
			if (name.length < opts.minimumNameLength) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.INVALID_NAME_LENGTH);
			if (name.length > opts.maximumNameLength) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.INVALID_NAME_LENGTH);
		} else if (opts.requireName) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.NAME_REQUIRED);
...
function deleteAllExpiredApiKeysEndpoint({ deleteAllExpiredApiKeys }) {
	return createAuthEndpoint$1({ method: "POST" }, async (ctx) => {
		try {
...
function deleteApiKey({ configurations, schema, deleteAllExpiredApiKeys }) {
	return createAuthEndpoint$1("/api-key/delete", {
		method: "POST",
		...
		if (session.user.banned === true) throw APIError$2.from("UNAUTHORIZED", API_KEY_ERROR_CODES.USER_BANNED);
		const lookupOpts = resolveConfiguration(ctx.context, configurations, configId);
		let apiKey = null;
		apiKey = await getApiKeyById(ctx, keyId, lookupOpts);
		if (!apiKey) throw APIError$2.from("NOT_FOUND", API_KEY_ERROR_CODES.KEY_NOT_FOUND);
		if (!configIdMatches(apiKey.configId, lookupOpts.configId)) throw APIError$2.from("NOT_FOUND", API_KEY_ERROR_CODES.KEY_NOT_FOUND);
		...
		else if (apiKey.referenceId !== session.user.id) throw APIError$2.from("NOT_FOUND", API_KEY_ERROR_CODES.KEY_NOT_FOUND);
		try {
			if (opts.storage === "secondary-storage" && opts.fallbackToDatabase) {
			...
			throw APIError$2.fromStatus("INTERNAL_SERVER_ERROR", { message: error?.message });
		}
		deleteAllExpiredApiKeys(ctx.context);
...
function getApiKey({ configurations, schema, deleteAllExpiredApiKeys }) {
	return createAuthEndpoint$1("/api-key/get", {
		method: "GET",
...
function listApiKeys({ configurations, schema, deleteAllExpiredApiKeys }) {
	return createAuthEndpoint$1("/api-key/list", {
		method: "GET",
		use: [sessionMiddleware],
...
function updateApiKey({ configurations, schema, deleteAllExpiredApiKeys }) {
	return createAuthEndpoint$1("/api-key/update", {
		method: "POST",
		...
		if (!user?.id) throw APIError$2.from("UNAUTHORIZED", API_KEY_ERROR_CODES.UNAUTHORIZED_SESSION);
		if (session && ctx.body.userId && session?.user.id !== ctx.body.userId) throw APIError$2.from("UNAUTHORIZED", API_KEY_ERROR_CODES.UNAUTHORIZED_SESSION);
		if (authRequired) {
			if (refillAmount !== void 0 || refillInterval !== void 0 || rateLimitMax !== void 0 || rateLimitTimeWindow !== void 0 || rateLimitEnabled !== void 0 || remaining !== void 0 || permissions !== void 0) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.SERVER_ONLY_PROPERTY);
		}
		const lookupOpts = resolveConfiguration(ctx.context, configurations, configId);
		let apiKey = null;
		apiKey = await getApiKeyById(ctx, keyId, lookupOpts);
		...
		else if (apiKey.referenceId !== user.id) throw APIError$2.from("NOT_FOUND", API_KEY_ERROR_CODES.KEY_NOT_FOUND);
		const newValues = {};
		if (name !== void 0) {
			if (name.length < opts.minimumNameLength) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.INVALID_NAME_LENGTH);
			else if (name.length > opts.maximumNameLength) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.INVALID_NAME_LENGTH);
			newValues.name = name;
		}
		if (enabled !== void 0) newValues.enabled = enabled;
		if (expiresIn !== void 0) {
			if (opts.keyExpiration.disableCustomExpiresTime === true) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.KEY_DISABLED_EXPIRATION);
			if (expiresIn !== null) {
				const expiresIn_in_days = expiresIn / (3600 * 24);
				if (expiresIn_in_days < opts.keyExpiration.minExpiresIn) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.EXPIRES_IN_IS_TOO_SMALL);
				else if (expiresIn_in_days > opts.keyExpiration.maxExpiresIn) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.EXPIRES_IN_IS_TOO_LARGE);
			}
		...
		if (refillAmount !== void 0 || refillInterval !== void 0) {
			if (refillAmount !== void 0 && refillInterval === void 0) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.REFILL_AMOUNT_AND_INTERVAL_REQUIRED);
			else if (refillInterval !== void 0 && refillAmount === void 0) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.REFILL_INTERVAL_AND_AMOUNT_REQUIRED);
		...
		if (Object.keys(newValues).length === 0) throw APIError$2.from("BAD_REQUEST", API_KEY_ERROR_CODES.NO_VALUES_TO_UPDATE);
...
async function validateApiKey({ hashedKey, ctx, opts, schema, permissions }) {
	...
	if (!apiKey) throw APIError$2.from("UNAUTHORIZED", API_KEY_ERROR_CODES.INVALID_API_KEY);
	if (apiKey.enabled === false) throw APIError$2.from("UNAUTHORIZED", API_KEY_ERROR_CODES.KEY_DISABLED);
	if (apiKey.expiresAt) {
		if (Date.now() > new Date(apiKey.expiresAt).getTime()) {
			...
			throw APIError$2.from("UNAUTHORIZED", API_KEY_ERROR_CODES.KEY_EXPIRED);
		}
	}
	if (permissions) {
		const apiKeyPermissions = apiKey.permissions ? safeJSONParse(apiKey.permissions) : null;
		if (!apiKeyPermissions) throw APIError$2.from("UNAUTHORIZED", API_KEY_ERROR_CODES.KEY_NOT_FOUND);
		if (!role(apiKeyPermissions).authorize(permissions).success) throw APIError$2.from("UNAUTHORIZED", API_KEY_ERROR_CODES.KEY_NOT_FOUND);
	}
	...
	if (apiKey.remaining === 0 && apiKey.refillAmount === null) {
		...
		throw APIError$2.from("TOO_MANY_REQUESTS", API_KEY_ERROR_CODES.USAGE_EXCEEDED);
		...
		if (remaining === 0) throw APIError$2.from("TOO_MANY_REQUESTS", API_KEY_ERROR_CODES.USAGE_EXCEEDED);
		else remaining--;
	}
	const { message, success, update, tryAgainIn } = isRateLimited(apiKey, opts);
	if (success === false) throw new APIError$2("UNAUTHORIZED", {
	...
	if (opts.deferUpdates) {
		...
		if (!newApiKey) throw APIError$2.from("INTERNAL_SERVER_ERROR", API_KEY_ERROR_CODES.FAILED_TO_UPDATE_API_KEY);
	}
	return newApiKey;
}
...
//# sourceMappingURL=@better-auth_api-key.js.map