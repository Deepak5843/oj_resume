...
var hslaStringToHsva = (hslString) => {
	var match = /hsla?\(?\s*(-?\d*\.?\d+)(deg|rad|grad|turn)?[,\s]+(-?\d*\.?\d+)%?[,\s]+(-?\d*\.?\d+)%?,?\s*[/\s]*(-?\d*\.?\d+)?(%)?\s*\)?/i.exec(hslString);
	if (!match) return {
		h: 0,
		s: 0,
		v: 0,
...
var hsvaStringToHsva = (hsvString) => {
	var match = /hsva?\(?\s*(-?\d*\.?\d+)(deg|rad|grad|turn)?[,\s]+(-?\d*\.?\d+)%?[,\s]+(-?\d*\.?\d+)%?,?\s*[/\s]*(-?\d*\.?\d+)?(%)?\s*\)?/i.exec(hsvString);
	if (!match) return {
		h: 0,
		s: 0,
		v: 0,
...
*/
var xyToRgb = (_ref11) => {
	var { x, y, bri = 255 } = _ref11;
	var red = x * 3.2406255 + y * -1.537208 + bri * -.4986286;
	var green = x * -.9689307 + y * 1.8757561 + bri * .0415175;
	var blue = x * .0557101 + y * -.2040211 + bri * 1.0569959;
	var translate = function translate(color) {
		return color <= .0031308 ? 12.92 * color : 1.055 * Math.pow(color, 1 / 2.4) - .055;
	};
	return {
...
var getContrastingColor = (str) => {
	...
	var col = color(str);
	return (col.rgb.r * 299 + col.rgb.g * 587 + col.rgb.b * 114) / 1e3 >= 128 ? "#000000" : "#ffffff";
};
var equalColorObjects = (first, second) => {
	if (first === second) return true;
	for (var prop in first) if (first[prop] !== second[prop]) return false;
	return true;
};
...
//#endregion
export { rgbToXY as A, hsvaToHsvaString as C, parseHue as D, hsvaToRgbaString as E, rgbaToRgb as F, validHex as I, xyToHex as L, rgbaToHex as M, rgbaToHexa as N, rgbStringToHsva as O, rgbaToHsva as P, xyToRgb as R, hsvaToHsvString as S, hsvaToRgba as T, hsvaToHexa as _, getContrastingColor as a, hsvaToHslaString as b, hexToXY as c, hslaStringToHsva as d, hslaToHsl as f, hsvaToHex as g, hsvaStringToHsva as h, equalHex as i, rgbaStringToHsva as j, rgbToHex as k, hslStringToHsla as l, hsvStringToHsva as m, equalColorObjects as n, hexToHsva as o, hslaToHsva as p, equalColorString as r, hexToRgba as s, color as t, hslStringToHsva as u, hsvaToHslString as v, hsvaToRgbString as w, hsvaToHsv as x, hsvaToHsla as y, _extends as z };

//# sourceMappingURL=esm-C2Dc3xiG.js.map