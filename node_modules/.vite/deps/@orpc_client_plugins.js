import { F as stringifyJSON, I as toArray, M as splitInHalf, R as value, g as isAsyncIteratorObject, n as AsyncIteratorClass, u as defer, w as overlayProxy } from "./dist-D6AwnVLs.js";
import { a as flattenHeader, f as COMMON_ORPC_ERROR_DEFS, s as getEventMeta, u as replicateStandardLazyResponse } from "./dist-Cr8nxtrC.js";
import { i as toBatchRequest, n as parseBatchResponse, r as toBatchAbortSignal } from "./batch-CrHXSC1a.js";
//#region node_modules/@orpc/client/dist/plugins/index.mjs
var BatchLinkPlugin = class {
	groups;
	maxSize;
	batchUrl;
	maxUrlLength;
	...
	constructor(options) {
		...
		this.batchHeaders = options.headers ?? (([options2, ...rest]) => {
			const headers = {};
			for (const [key, value2] of Object.entries(options2.request.headers)) if (rest.every((item) => item.request.headers[key] === value2)) headers[key] = value2;
			return headers;
		});
		this.mapRequestItem = options.mapRequestItem ?? (({ request, batchHeaders }) => {
			const headers = {};
			for (const [key, value2] of Object.entries(request.headers)) if (batchHeaders[key] !== value2) headers[key] = value2;
			return {
	...
	init(options) {
		...
		options.clientInterceptors.push((options2) => {
			if (options2.request.headers["x-orpc-batch"] !== "1") return options2.next();
		...
		options.clientInterceptors.push((options2) => {
			if (this.exclude(options2) || options2.request.body instanceof Blob || options2.request.body instanceof FormData || isAsyncIteratorObject(options2.request.body) || options2.request.signal?.aborted) return options2.next();
			const group = this.groups.find((group2) => group2.condition(options2));
			if (!group) return options2.next();
	...
	async #executeBatch(method, group, groupItems) {
		...
		try {
			...
			const batchRequest = toBatchRequest({
				...
				requests: batchItems.map(([options2]) => this.mapRequestItem({
					...options2,
					batchUrl,
					batchHeaders
				}))
			...
			try {
				...
				const parsed = parseBatchResponse({
					...lazyResponse,
					body: await lazyResponse.body()
				});
				for await (const item of parsed) batchItems[item.index]?.[1]({
					...item,
					body: () => Promise.resolve(item.body)
				});
			} catch (err) {
				if (batchRequest.signal?.aborted && batchRequest.signal.reason === err) {
					for (const [{ signal }, , reject] of batchItems) if (signal?.aborted) reject(signal.reason);
				}
				throw err;
			}
			...
			for (const [, , reject] of batchItems) reject(error);
		}
	}
};
var DedupeRequestsPlugin = class {
	#groups;
	#filter;
	order = 4e6;
	...
	init(options) {
		options.clientInterceptors ??= [];
		options.clientInterceptors.push((options2) => {
			if (options2.request.body instanceof Blob || options2.request.body instanceof FormData || options2.request.body instanceof URLSearchParams || isAsyncIteratorObject(options2.request.body) || !this.#filter(options2)) return options2.next();
			const group = this.#groups.find((group2) => group2.condition(options2));
			if (!group) return options2.next();
			return new Promise((resolve, reject) => {
		...
		if (!queue) this.#queue.set(group, queue = []);
	...
	async #dequeue() {
		const promises = [];
		for (const [group, items] of this.#queue) for (const { options, signals, resolves, rejects } of items) promises.push(this.#execute(group, options, signals, resolves, rejects));
		this.#queue.clear();
	...
	async #execute(group, options, signals, resolves, rejects) {
		try {
			...
			const replicatedResponses = replicateStandardLazyResponse(await options.next({
				...options,
				request: dedupedRequest,
				signal: dedupedRequest.signal,
				context: group.context
			}), resolves.length);
			for (const resolve of resolves) resolve(replicatedResponses.shift());
		} catch (error) {
...
var RetryAfterPlugin = class {
	...
	constructor(options = {}) {
		...
		this.timeout = options.timeout ?? 300 * 1e3;
	}
	init(options) {
		options.clientInterceptors ??= [];
		options.clientInterceptors.push(async (interceptorOptions) => {
			...
			while (true) {
				...
				if (!value(this.condition, response, interceptorOptions)) return response;
				...
				if (retryAfterMs === void 0) return response;
				if (attemptCount >= value(this.maxAttempts, response, interceptorOptions)) return response;
				const timeoutMs = value(this.timeout, response, interceptorOptions);
				if (Date.now() - startTime + retryAfterMs > timeoutMs) return response;
				await this.delayExecution(retryAfterMs, interceptorOptions.signal);
				if (interceptorOptions.signal?.aborted) return response;
			}
		});
	}
...
var SimpleCsrfProtectionLinkPlugin = class {
	headerName;
	headerValue;
	exclude;
	constructor(options = {}) {
	...
	init(options) {
		...
		options.clientInterceptors.push(async (options2) => {
			if (await value(this.exclude, options2)) return options2.next();
...
//# sourceMappingURL=@orpc_client_plugins.js.map