...
import { C as SqliteDialect, G as Kysely, c as MssqlDialect, h as MysqlDialect, p as PostgresDialect, z as sql } from "./esm-BOuo2Wv3.js";
import { n as createAdapterFactory } from "./adapter-BIY3jVDm.js";
...
var createKyselyAdapter = async (config) => {
	...
	if ("fileControl" in db) {
		const { BunSqliteDialect } = await import("./bun-sqlite-dialect-C8OaCWSL-CSkZ5w7R.js");
	...
	if ("createSession" in db) {
		...
		if (DatabaseSync && db instanceof DatabaseSync) {
			const { NodeSqliteDialect } = await import("./node-sqlite-dialect-aB5Saivy.js");
			dialect = new NodeSqliteDialect({ database: db });
		}
	}
	if ("batch" in db && "exec" in db && "prepare" in db) {
		const { D1SqliteDialect } = await import("./d1-sqlite-dialect-sYHNqBte-CdtOODLm.js");
