import { F as stringifyJSON, T as parseEmptyableJSON, _ as isObject, g as isAsyncIteratorObject, n as AsyncIteratorClass } from "./dist-D6AwnVLs.js";
...
function toBatchAbortSignal(signals) {
	...
	if (realSignals.length === 0 || realSignals.length !== signals.length) return;
	const controller = new AbortController();
	const abortIfAllInputsAborted = () => {
		if (realSignals.every((signal) => signal.aborted)) controller.abort();
	};
	abortIfAllInputsAborted();
	for (const signal of realSignals) signal.addEventListener("abort", () => {
		abortIfAllInputsAborted();
	}, {
		once: true,
...
function toBatchRequest(options) {
	...
	if (options.method === "GET") url.searchParams.append("batch", stringifyJSON(batchRequestItems));
	else if (options.method === "POST") body = batchRequestItems;
	return {
	...
	if (isAsyncIteratorObject(body) || Array.isArray(body)) {
		const iterator = (async function* () {
			for await (const item of body) {
				if (!isObject(item) || !("index" in item) || typeof item.index !== "number") throw new TypeError("Invalid batch response", { cause: item });
				yield {
					index: item.index,
		...
		return new AsyncIteratorClass(() => iterator.next(), async (reason) => {
			if (reason !== "next" && isAsyncIteratorObject(body)) await body.return?.();
		});
	}
	throw new TypeError("Invalid batch response", { cause: response });
}
//#endregion
export { toBatchResponse as a, toBatchRequest as i, parseBatchResponse as n, toBatchAbortSignal as r, parseBatchRequest as t };

//# sourceMappingURL=batch-CrHXSC1a.js.map