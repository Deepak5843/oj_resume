import { An as registry, kn as globalRegistry } from "./core-Bn-bagXs.js";
import { i as guard, n as JsonSchemaXNativeType } from "./dist-BbnGd9WN.js";
import { I as toArray, _ as isObject, h as intercept } from "./dist-D6AwnVLs.js";
import "./dist-Cr8nxtrC.js";
import "./client.BLtwTQUg-DfwNBSJl.js";
import "./client.L87CEugX-BUAqOM0-.js";
import "./dist-XIz-cOri.js";
import "./standard-D0x0IPzC.js";
import "./server.Ds4HPpvH-BBUMdhgg.js";
import { D as ContentEncoding, O as Format } from "./dist-Bn8jpnhU.js";
//#region node_modules/@orpc/zod/dist/zod4/index.mjs
var experimental_ZodSmartCoercionPlugin = class {
	init(options) {
		options.clientInterceptors ??= [];
		options.clientInterceptors.unshift((options2) => {
			const inputSchema = options2.procedure["~orpc"].inputSchema;
			if (!inputSchema || inputSchema["~standard"].vendor !== "zod" || !("_zod" in inputSchema)) return options2.next();
	...
	#coerce(schema, value) {
		switch (schema._zod.def.type) {
			case "number":
				...
				return value;
			case "bigint":
				if (typeof value === "string") return this.#stringToBigInt(value);
				return value;
			case "boolean":
			case "success":
				if (typeof value === "string") return this.#stringToBoolean(value);
				return value;
			case "date":
				if (typeof value === "string") return this.#stringToDate(value);
				return value;
			case "literal":
			case "enum": {
				...
				if (!literal._zod.values.has(value) && typeof value === "string") {
					...
					if (literal._zod.values.has(num)) return num;
					const bool = this.#stringToBoolean(value);
					if (literal._zod.values.has(bool)) return bool;
					const bigint = this.#stringToBigInt(value);
					if (literal._zod.values.has(bigint)) return bigint;
				}
				return value;
			}
			case "array": {
				...
				if (Array.isArray(value)) return value.map((v) => this.#coerce(array._zod.def.element, v));
				return value;
			}
			case "tuple": {
				const tuple = schema;
			...
			case "set": {
				...
				if (Array.isArray(value)) return new Set(value.map((v) => this.#coerce(set._zod.def.valueType, v)));
				if (value instanceof Set) return new Set(Array.from(value).map((v) => this.#coerce(set._zod.def.valueType, v)));
				return value;
			}
			case "object": {
				const object = schema;
			...
			case "record": {
				...
				if (isObject(value)) {
					...
					for (const [k, v] of Object.entries(value)) {
						...
						newObj[key] = this.#coerce(record._zod.def.valueType, v);
					}
					return newObj;
				}
				return value;
			...
			case "map": {
				...
				if (Array.isArray(value) && value.every((i) => Array.isArray(i) && i.length <= 2)) return new Map(value.map(([k, v]) => [this.#coerce(map._zod.def.keyType, k), this.#coerce(map._zod.def.valueType, v)]));
				if (value instanceof Map) return new Map(Array.from(value).map(([k, v]) => [this.#coerce(map._zod.def.keyType, k), this.#coerce(map._zod.def.valueType, v)]));
				return value;
			}
			case "union": {
				const union = schema;
				if (union._zod.def.options.length === 1) return this.#coerce(union._zod.def.options[0], value);
				if (isObject(value)) {
					...
					for (const option of union._zod.def.options) {
						...
						if (discriminator !== void 0) {
							if (option._zod.propValues[discriminator]?.has(value[discriminator])) return this.#coerce(option, value);
						} else for (const key in option._zod.propValues) if (option._zod.propValues[key]?.has(value[key])) return this.#coerce(option, value);
					}
				}
				return value;
			}
			...
			case "lazy": {
				const lazy = schema;
				if (value !== void 0) return this.#coerce(lazy._zod.def.getter(), value);
				return value;
			}
			default:
	...
	#stringToBoolean(value) {
		...
		if (lower === "false" || lower === "off" || lower === "f") return false;
		if (lower === "true" || lower === "on" || lower === "t") return true;
		return value;
	}
	#stringToDate(value) {
...
//# sourceMappingURL=@orpc_zod_zod4.js.map