...
function defer(callback) {
	...
	else Promise.resolve().then(() => Promise.resolve().then(() => Promise.resolve().then(callback)));
}
...
var AsyncIdQueue = class {
	...
	assertOpen(id) {
		if (!this.isOpen(id)) throw new Error(`[AsyncIdQueue] Cannot access queue[${id}] because it is not open or aborted.`);
	}
};
	...
	const start = once(async () => {
		try {
			while (true) {
				...
				ids.forEach((id) => {
					if (queue.isOpen(id)) queue.push(id, { next: item });
				});
				if (item.done) break;
			}
		} catch (error) {
			ids.forEach((id) => {
				if (queue.isOpen(id)) queue.push(id, { error });
			});
		} finally {
	...
	*/
	...
	subscribe(event, listenerOrOptions) {
		if (typeof listenerOrOptions === "function") {
			...
			if (!listeners) this.#listenersMap.set(event, listeners = []);
			listeners.push(listenerOrOptions);
			return once(() => {
				listeners.splice(listeners.indexOf(listenerOrOptions), 1);
				if (listeners.length === 0) this.#listenersMap.delete(event);
			});
		}
		...
		return new AsyncIteratorClass(async () => {
			...
			pullResolvers.forEach((resolver) => resolver[0]({
				done: true,
				value: void 0
			}));
			pullResolvers.length = 0;
...
//#endregion
export { runWithSpan as A, once as C, replicateAsyncIterator as D, preventNativeAwait as E, stringifyJSON as F, toArray as I, tryDecodeURIComponent as L, splitInHalf as M, startSpan as N, resolveMaybeOptionalOptions as O, streamToAsyncIteratorClass as P, value as R, onSuccess as S, parseEmptyableJSON as T, isObject as _, ORPC_NAME as a, onFinish as b, asyncIteratorWithSpan as c, findDeepMatches as d, get as f, isAsyncIteratorObject as g, intercept as h, NullProtoObj as i, setSpanError as j, runInSpanContext as k, clone as l, getGlobalOtelConfig as m, AsyncIteratorClass as n, asyncIteratorToStream as o, getConstructor as p, EventPublisher as r, asyncIteratorToUnproxiedDataStream as s, AbortError as t, defer as u, isTypescriptObject as v, overlayProxy as w, onStart as x, onError as y };

//# sourceMappingURL=dist-D6AwnVLs.js.map