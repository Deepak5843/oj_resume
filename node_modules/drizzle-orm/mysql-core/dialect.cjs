...
var MySqlDialect = class {
	...
	*/
	...
	buildSelectQuery({ withList, fields, fieldsFlat, where, having, table, joins, orderBy, groupBy, limit, offset, lockingClause, distinct, setOperators, useIndex, forceIndex, ignoreIndex }) {
		...
		const finalQuery = __sql_sql_ts.sql`${withSql}select${distinctSql} ${selection} from ${tableSql}${useIndexSql}${forceIndexSql}${ignoreIndexSql}${joinsSql}${whereSql}${groupBySql}${havingSql}${orderBySql}${limitSql}${offsetSql}${lockingClausesSql}`;
	...
	buildInsertQuery({ table, values: valuesOrSelect, ignore, onConflict, select }) {
		...
		return {
			sql: __sql_sql_ts.sql`insert${ignore ? __sql_sql_ts.sql` ignore` : void 0} into ${table} ${insertOrder} ${valuesSql}${onConflict ? __sql_sql_ts.sql` on duplicate key ${onConflict}` : void 0}`,
