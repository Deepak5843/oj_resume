import { h as getRequestHeaders, m as getCookie, v as setCookie$1 } from "./esm-m5_tZiEi.js";
import { a as TWO_FACTOR_ERROR_CODES, c as getHostFromRequest, d as getProtocolFromRequest, f as isDynamicBaseURLConfig, g as wildcardMatch, h as resolveDynamicBaseURL, l as getOrigin, m as resolveBaseURL, n as GENERIC_OAUTH_ERROR_CODES, o as getBaseURL, p as matchesHostPattern, r as USERNAME_ERROR_CODES, s as getHost, u as getProtocol } from "./error-codes-HOUJ9XOX.js";
import { i as slugify, n as generateRandomName, o as toUsername, t as generateId$1 } from "./string-DcW5i2yt.js";
import { E as printMarginTemplates, b as resumeDataSchema, s as defaultResumeData } from "./data-CIU9AXln.js";
import { t as getLocale } from "./locale-Bc_Sesmg.js";
...
import { APIError, APIError as APIError$2, BASE_ERROR_CODES, BetterAuthError } from "@better-auth/core/error";
import { createRandomStringGenerator } from "@better-auth/utils/random";
import { hkdf } from "@noble/hashes/hkdf.js";
import { sha256 } from "@noble/hashes/sha2.js";
import { EncryptJWT, SignJWT, base64url, calculateJwkThumbprint, decodeJwt, decodeProtectedHeader, jwtDecrypt, jwtVerify } from "jose";
...
import * as import__better_auth_core_db$1 from "@better-auth/core/db";
import { getAuthTables } from "@better-auth/core/db";
import { filterOutputFields } from "@better-auth/core/utils/db";
import { safeJSONParse } from "@better-auth/core/utils/json";
import * as z$2 from "zod";
import z$1, { ZodError, flattenError, z } from "zod";
import { base64Url } from "@better-auth/utils/base64";
import { binary } from "@better-auth/utils/binary";
import { createHMAC } from "@better-auth/utils/hmac";
import { defineRequestState, getBetterAuthVersion, getCurrentAdapter, getCurrentAdapter as getCurrentAdapter$1, getCurrentAuthContext, hasRequestState, queueAfterTransactionHook, runWithAdapter, runWithEndpointContext, runWithRequestState, runWithTransaction } from "@better-auth/core/context";
import { APIError as APIError$1, createRouter, kAPIErrorHeaderSymbol, toResponse } from "better-call";
...
import { createTelemetry, createTelemetry as createTelemetry$1, getTelemetryAuthConfig } from "@better-auth/telemetry";
...
import { and, arrayContains, asc, count, defineRelations, desc, eq, isNotNull, or, sql as sql$1 } from "drizzle-orm";
...
import { ORPCError as ORPCError$1, createRouterClient, os, streamToEventIterator, type } from "@orpc/server";
import { createTanstackQueryUtils } from "@orpc/tanstack-query";
import { AISDKError, Output, convertToModelMessages, createGateway, generateText, stepCountIs, streamText, tool } from "ai";
import { OllamaError, createOllama } from "ai-sdk-ollama";
...
import { createHash as createHash$1, timingSafeEqual } from "node:crypto";
...
var getSessionQuerySchema$1 = z$2.optional(z$2.object({
...
function toZodSchema({ fields, isClientSide }) {
	const zodFields = Object.keys(fields).reduce((acc, key) => {
		...
		let schema;
		if (field.type === "json") schema = z$2.json ? z$2.json() : z$2.any();
		else if (field.type === "string[]" || field.type === "number[]") schema = z$2.array(field.type === "string[]" ? z$2.string() : z$2.number());
		else if (Array.isArray(field.type)) schema = z$2.any();
		else schema = z$2[field.type]();
...
__reExport(db_exports, import__better_auth_core_db$1);
//#endregion
...
var accountInfoQuerySchema = z$2.optional(z$2.object({ accountId: z$2.string().meta({ description: "The provider given account id for which to get the account info" }).optional() }));
...
async function getAdapter(options) {
	return getBaseAdapter(options, async (opts) => {
		const { createKyselyAdapter } = await import("./kysely-adapter-CV9OHGtH.js");
		...
		const { kyselyAdapter } = await import("./kysely-adapter-CV9OHGtH.js");
...
z$2.object({
	userId: z$2.coerce.string().meta({ description: "The user id" }),
	role: z$2.union([z$2.string().meta({ description: "The role to set. `admin` or `user` by default" }), z$2.array(z$2.string().meta({ description: "The roles to set. `admin` or `user` by default" }))]).meta({ description: "The role to set, this can be a string or an array of strings. Eg: `admin` or `[admin, user]`" })
});
z$2.object({ id: z$2.string().meta({ description: "The id of the User" }) });
z$2.object({
	email: z$2.string().meta({ description: "The email of the user" }),
	password: z$2.string().optional().meta({ description: "The password of the user. If not provided, the user will be created without a credential account (useful for magic link or social login only users)." }),
	name: z$2.string().meta({ description: "The name of the user" }),
	role: z$2.union([z$2.string().meta({ description: "The role of the user" }), z$2.array(z$2.string().meta({ description: "The roles of user" }))]).optional().meta({ description: `A string or array of strings representing the roles to apply to the new user. Eg: \"user\"` }),
	data: z$2.record(z$2.string(), z$2.any()).optional().meta({ description: "Extra fields for the user. Including custom additional fields." })
});
z$2.object({
	userId: z$2.coerce.string().meta({ description: "The user id" }),
	data: z$2.record(z$2.any(), z$2.any()).meta({ description: "The user data to update" })
});
z$2.object({
	searchValue: z$2.string().optional().meta({ description: "The value to search for. Eg: \"some name\"" }),
	searchField: z$2.enum(["email", "name"]).meta({ description: "The field to search in, defaults to email. Can be `email` or `name`. Eg: \"name\"" }).optional(),
	searchOperator: z$2.enum([
		"contains",
		"starts_with",
		"ends_with"
	]).meta({ description: "The operator to use for the search. Can be `contains`, `starts_with` or `ends_with`. Eg: \"contains\"" }).optional(),
	limit: z$2.string().meta({ description: "The number of users to return" }).or(z$2.number()).optional(),
	offset: z$2.string().meta({ description: "The offset to start from" }).or(z$2.number()).optional(),
	sortBy: z$2.string().meta({ description: "The field to sort by" }).optional(),
	sortDirection: z$2.enum(["asc", "desc"]).meta({ description: "The direction to sort by" }).optional(),
	filterField: z$2.string().meta({ description: "The field to filter by" }).optional(),
	filterValue: z$2.string().meta({ description: "The value to filter by" }).or(z$2.number()).or(z$2.boolean()).or(z$2.array(z$2.string())).or(z$2.array(z$2.number())).optional(),
	filterOperator: z$2.enum(whereOperators).meta({ description: "The operator to use for the filter" }).optional()
});
z$2.object({ userId: z$2.coerce.string().meta({ description: "The user id" }) });
z$2.object({ userId: z$2.coerce.string().meta({ description: "The user id" }) });
z$2.object({
	userId: z$2.coerce.string().meta({ description: "The user id" }),
	banReason: z$2.string().meta({ description: "The reason for the ban" }).optional(),
	banExpiresIn: z$2.number().meta({ description: "The number of seconds until the ban expires" }).optional()
});
z$2.object({ userId: z$2.coerce.string().meta({ description: "The user id" }) });
z$2.object({ sessionToken: z$2.string().meta({ description: "The session token" }) });
z$2.object({ userId: z$2.coerce.string().meta({ description: "The user id" }) });
z$2.object({ userId: z$2.coerce.string().meta({ description: "The user id" }) });
z$2.object({
	newPassword: z$2.string().nonempty("newPassword cannot be empty").meta({ description: "The new password" }),
	userId: z$2.coerce.string().nonempty("userId cannot be empty").meta({ description: "The user id" })
});
z$2.object({
	userId: z$2.coerce.string().optional().meta({ description: `The user id. Eg: "user-id"` }),
	role: z$2.string().optional().meta({ description: `The role to check permission for. Eg: "admin"` })
}).and(z$2.union([z$2.object({
	permission: z$2.record(z$2.string(), z$2.array(z$2.string())),
	permissions: z$2.undefined()
}), z$2.object({
	permission: z$2.undefined(),
	permissions: z$2.record(z$2.string(), z$2.array(z$2.string()))
})]));
//#endregion
...
z$2.optional(z$2.object({
	disableCookieCache: z$2.boolean().meta({ description: "Disable cookie cache and fetch session from database" }).or(z$2.string().transform((v) => v === "true")).optional(),
	disableRefresh: z$2.boolean().meta({ description: "Disable session refresh. Useful for checking session status, without updating the session" }).optional()
}));
//#endregion
...
z$2.object({
	client_id: z$2.string().meta({ description: "The client ID of the application" }),
	scope: z$2.string().meta({ description: "Space-separated list of scopes" }).optional()
});
z$2.object({
	error: z$2.enum(["invalid_request", "invalid_client"]).meta({ description: "Error code" }),
	error_description: z$2.string().meta({ description: "Detailed error description" })
});
z$2.object({
	grant_type: z$2.literal("urn:ietf:params:oauth:grant-type:device_code").meta({ description: "The grant type for device flow" }),
	device_code: z$2.string().meta({ description: "The device verification code" }),
	client_id: z$2.string().meta({ description: "The client ID of the application" })
});
z$2.object({
	error: z$2.enum([
...
createAuthEndpoint("/device", {
	method: "GET",
	query: z$2.object({ user_code: z$2.string().meta({ description: "The user code to verify" }) }),
	error: z$2.object({
		error: z$2.enum(["invalid_request"]).meta({ description: "Error code" }),
		error_description: z$2.string().meta({ description: "Detailed error description" })
	}),
	metadata: { openapi: {
...
createAuthEndpoint("/device/approve", {
	method: "POST",
	body: z$2.object({ userCode: z$2.string().meta({ description: "The user code to approve" }) }),
	error: z$2.object({
		error: z$2.enum([
			"invalid_request",
			"expired_token",
...
createAuthEndpoint("/device/deny", {
	method: "POST",
	body: z$2.object({ userCode: z$2.string().meta({ description: "The user code to deny" }) }),
	error: z$2.object({
		error: z$2.enum([
			"invalid_request",
			"expired_token",
...
z$2.object({
	id: z$2.string(),
	deviceCode: z$2.string(),
	userCode: z$2.string(),
	userId: z$2.string().optional(),
	expiresAt: z$2.date(),
	status: z$2.string(),
	lastPolledAt: z$2.date().optional(),
	pollingInterval: z$2.number().optional(),
	clientId: z$2.string().optional(),
	scope: z$2.string().optional()
});
//#endregion
//#region node_modules/better-auth/dist/plugins/device-authorization/index.mjs
var timeStringSchema = z$2.custom((val) => {
...
z$2.object({
	...
	deviceCodeLength: z$2.number().int().positive().default(40).describe("Length of the device code to be generated. Default is 40 characters."),
	userCodeLength: z$2.number().int().positive().default(8).describe("Length of the user code to be generated. Default is 8 characters."),
	generateDeviceCode: z$2.custom((val) => typeof val === "function", { message: "generateDeviceCode must be a function that returns a string or a promise that resolves to a string." }).optional().describe("Function to generate a device code. If not provided, a default random string generator will be used."),
	generateUserCode: z$2.custom((val) => typeof val === "function", { message: "generateUserCode must be a function that returns a string or a promise that resolves to a string." }).optional().describe("Function to generate a user code. If not provided, a default random string generator will be used."),
	validateClient: z$2.custom((val) => typeof val === "function", { message: "validateClient must be a function that returns a boolean or a promise that resolves to a boolean." }).optional().describe("Function to validate the client ID. If not provided, no validation will be performed."),
	onDeviceAuthRequest: z$2.custom((val) => typeof val === "function", { message: "onDeviceAuthRequest must be a function that returns void or a promise that resolves to void." }).optional().describe("Function to handle device authorization requests. If not provided, no additional actions will be taken."),
	verificationUri: z$2.string().optional().describe("The URI where users verify their device code. Can be an absolute URL (https://example.com/device) or relative path (/custom-path). This will be returned as verification_uri in the device code response. If not provided, defaults to /device."),
	schema: z$2.custom(() => true)
});
//#endregion
...
z$2.object({
	email: z$2.string({}).meta({ description: "Email address to send the OTP" }),
	type: z$2.enum(types).meta({ description: "Type of the OTP" })
});
z$2.object({
	email: z$2.string({}).meta({ description: "Email address to send the OTP" }),
	type: z$2.enum(types).meta({
		required: true,
		description: "Type of the OTP"
	})
});
z$2.object({
	email: z$2.string({}).meta({ description: "Email address the OTP was sent to" }),
	type: z$2.enum(types).meta({
		required: true,
		description: "Type of the OTP"
	})
...
z$2.object({
	email: z$2.string({}).meta({ description: "Email address to verify" }),
	otp: z$2.string().meta({
		required: true,
		description: "OTP to verify"
	})
...
z$2.object({
	email: z$2.string({}).meta({ description: "Email address to sign in" }),
	otp: z$2.string().meta({
		required: true,
		description: "OTP sent to the email"
	}),
	name: z$2.string().meta({ description: "User display name. Only used if the user is registering for the first time. Eg: \"my-name\"" }).optional(),
	image: z$2.string().meta({ description: "User profile image URL. Only used if the user is registering for the first time." }).optional()
}).and(z$2.record(z$2.string(), z$2.any()));
z$2.object({ email: z$2.string().meta({ description: "Email address to send the OTP" }) });
z$2.object({ email: z$2.string().meta({ description: "Email address to send the OTP" }) });
z$2.object({
	email: z$2.string().meta({ description: "Email address to reset the password" }),
	otp: z$2.string().meta({ description: "OTP sent to the email" }),
	password: z$2.string().meta({ description: "New password" })
});
z$2.object({
	newEmail: z$2.string().meta({ description: "New email address to send the OTP" }),
	otp: z$2.string().optional().meta({ description: "OTP sent to the current email. This is required if changeEmail.verifyCurrentEmail option is set to true" })
});
z$2.object({
	newEmail: z$2.string().meta({ description: "New email address to verify and change to" }),
	otp: z$2.string().meta({ description: "OTP sent to the new email" })
});
//#endregion
//#region node_modules/better-auth/dist/plugins/generic-oauth/routes.mjs
var signInWithOAuth2BodySchema = z$2.object({
	providerId: z$2.string().meta({ description: "The provider ID for the OAuth provider" }),
	callbackURL: z$2.string().meta({ description: "The URL to redirect to after sign in" }).optional(),
	errorCallbackURL: z$2.string().meta({ description: "The URL to redirect to if an error occurs" }).optional(),
	newUserCallbackURL: z$2.string().meta({ description: "The URL to redirect to after login if the user is new. Eg: \"/welcome\"" }).optional(),
	disableRedirect: z$2.boolean().meta({ description: "Disable redirect" }).optional(),
	scopes: z$2.array(z$2.string()).meta({ description: "Scopes to be passed to the provider authorization request." }).optional(),
	requestSignUp: z$2.boolean().meta({ description: "Explicitly request sign-up. Useful when disableImplicitSignUp is true for this provider. Eg: false" }).optional(),
	additionalData: z$2.record(z$2.string(), z$2.any()).optional()
});
/**
* ### Endpoint
...
var signInWithOAuth2 = (options) => createAuthEndpoint("/sign-in/oauth2", {
	...
	if (!config) throw APIError$2.fromStatus("BAD_REQUEST", { message: `${GENERIC_OAUTH_ERROR_CODES.PROVIDER_CONFIG_NOT_FOUND} ${providerId}` });
	...
	if (!finalAuthUrl || !finalTokenUrl) throw APIError$2.from("BAD_REQUEST", GENERIC_OAUTH_ERROR_CODES.INVALID_OAUTH_CONFIGURATION);
...
var OAuth2CallbackQuerySchema = z$2.object({
	code: z$2.string().meta({ description: "The OAuth2 code" }).optional(),
	error: z$2.string().meta({ description: "The error message, if any" }).optional(),
	error_description: z$2.string().meta({ description: "The error description, if any" }).optional(),
	state: z$2.string().meta({ description: "The state parameter from the OAuth2 request" }).optional(),
	iss: z$2.string().meta({ description: "The issuer identifier" }).optional()
});
var oAuth2Callback = (options) => createAuthEndpoint("/oauth2/callback/:providerId", {
	...
	if (!providerId) throw APIError$2.from("BAD_REQUEST", GENERIC_OAUTH_ERROR_CODES.PROVIDER_ID_REQUIRED);
	const providerConfig = options.config.find((p) => p.providerId === providerId);
	if (!providerConfig) throw APIError$2.fromStatus("BAD_REQUEST", { message: `${GENERIC_OAUTH_ERROR_CODES.PROVIDER_CONFIG_NOT_FOUND} ${providerId}` });
	let tokens = void 0;
	...
	try {
		if (providerConfig.getToken) tokens = await providerConfig.getToken({
			...
			if (!finalTokenUrl) throw APIError$2.from("BAD_REQUEST", GENERIC_OAUTH_ERROR_CODES.INVALID_OAUTH_CONFIG);
	...
	if (!tokens) throw APIError$2.from("BAD_REQUEST", GENERIC_OAUTH_ERROR_CODES.INVALID_OAUTH_CONFIG);
...
var OAuth2LinkAccountBodySchema = z$2.object({
	providerId: z$2.string(),
	callbackURL: z$2.string(),
	scopes: z$2.array(z$2.string()).meta({ description: "Additional scopes to request when linking the account" }).optional(),
	errorCallbackURL: z$2.string().meta({ description: "The URL to redirect to if there is an error during the link process" }).optional()
});
/**
* ### Endpoint
...
var oAuth2LinkAccount = (options) => createAuthEndpoint("/oauth2/link", {
	...
	const session = c.context.session;
	if (!session) throw APIError$2.from("UNAUTHORIZED", GENERIC_OAUTH_ERROR_CODES.SESSION_REQUIRED);
	const provider = options.config.find((p) => p.providerId === c.body.providerId);
	if (!provider) throw APIError$2.from("NOT_FOUND", BASE_ERROR_CODES.PROVIDER_NOT_FOUND);
	...
	let finalAuthUrl = authorizationUrl;
	if (!finalAuthUrl) {
		if (!discoveryUrl) throw APIError$2.from("BAD_REQUEST", GENERIC_OAUTH_ERROR_CODES.INVALID_OAUTH_CONFIGURATION);
		const discovery = await betterFetch(discoveryUrl, {
			method: "GET",
	...
	if (!finalAuthUrl) throw APIError$2.from("BAD_REQUEST", GENERIC_OAUTH_ERROR_CODES.INVALID_OAUTH_CONFIGURATION);
	const state = await generateState(c, {
		userId: session.user.id,
		email: session.user.email
	}, void 0);
...
z$2.object({
	payload: z$2.record(z$2.string(), z$2.any()),
	overrideOptions: z$2.record(z$2.string(), z$2.any()).optional()
});
z$2.object({
	token: z$2.string(),
	issuer: z$2.string().optional()
});
z$2.object({
	email: z$2.email().meta({ description: "Email address to send the magic link" }),
	name: z$2.string().meta({ description: "User display name. Only used if the user is registering for the first time. Eg: \"my-name\"" }).optional(),
	callbackURL: z$2.string().meta({ description: "URL to redirect after magic link verification" }).optional(),
	newUserCallbackURL: z$2.string().meta({ description: "URL to redirect after new user signup. Only used if the user is registering for the first time." }).optional(),
	errorCallbackURL: z$2.string().meta({ description: "URL to redirect after error." }).optional()
});
z$2.object({
	token: z$2.string().meta({ description: "Verification token" }),
	callbackURL: z$2.string().meta({ description: "URL to redirect after magic link verification, if not provided the user will be redirected to the root URL. Eg: \"/dashboard\"" }).optional(),
	errorCallbackURL: z$2.string().meta({ description: "URL to redirect after error." }).optional(),
	newUserCallbackURL: z$2.string().meta({ description: "URL to redirect after new user signup. Only used if the user is registering for the first time." }).optional()
});
//#endregion
...
z$2.object({
	clientId: z$2.string(),
	clientSecret: z$2.string().optional(),
	type: z$2.enum([
		"web",
		"native",
		"user-agent-based",
	...
	metadata: z$2.string().optional(),
	disabled: z$2.boolean().optional().default(false),
	redirectUrls: z$2.string(),
	userId: z$2.string().optional(),
	createdAt: z$2.date(),
	updatedAt: z$2.date()
});
z$2.object({
	accept: z$2.boolean(),
	consent_code: z$2.string().optional().nullish()
});
z$2.record(z$2.any(), z$2.any());
z$2.object({
	redirect_uris: z$2.array(z$2.string()).meta({ description: "A list of redirect URIs. Eg: [\"https://client.example.com/callback\"]" }),
	token_endpoint_auth_method: z$2.enum([
		"none",
		"client_secret_basic",
		"client_secret_post"
	...
	grant_types: z$2.array(z$2.enum([
		"authorization_code",
		"implicit",
		"password",
		"client_credentials",
	...
	response_types: z$2.array(z$2.enum(["code", "token"])).meta({ description: "The response types supported by the application. Eg: [\"code\"]" }).default(["code"]).optional(),
	client_name: z$2.string().meta({ description: "The name of the application. Eg: \"My App\"" }).optional(),
	client_uri: z$2.string().meta({ description: "The URI of the application. Eg: \"https://client.example.com\"" }).optional(),
	logo_uri: z$2.string().meta({ description: "The URI of the application logo. Eg: \"https://client.example.com/logo.png\"" }).optional(),
	scope: z$2.string().meta({ description: "The scopes supported by the application. Separated by spaces. Eg: \"profile email\"" }).optional(),
	contacts: z$2.array(z$2.string()).meta({ description: "The contact information for the application. Eg: [\"admin@example.com\"]" }).optional(),
	tos_uri: z$2.string().meta({ description: "The URI of the application terms of service. Eg: \"https://client.example.com/tos\"" }).optional(),
	policy_uri: z$2.string().meta({ description: "The URI of the application privacy policy. Eg: \"https://client.example.com/policy\"" }).optional(),
	jwks_uri: z$2.string().meta({ description: "The URI of the application JWKS. Eg: \"https://client.example.com/jwks\"" }).optional(),
	jwks: z$2.record(z$2.any(), z$2.any()).meta({ description: "The JWKS of the application. Eg: {\"keys\": [{\"kty\": \"RSA\", \"alg\": \"RS256\", \"use\": \"sig\", \"n\": \"...\", \"e\": \"...\"}]}" }).optional(),
	metadata: z$2.record(z$2.any(), z$2.any()).meta({ description: "The metadata of the application. Eg: {\"key\": \"value\"}" }).optional(),
	software_id: z$2.string().meta({ description: "The software ID of the application. Eg: \"my-software\"" }).optional(),
	software_version: z$2.string().meta({ description: "The software version of the application. Eg: \"1.0.0\"" }).optional(),
	software_statement: z$2.string().meta({ description: "The software statement of the application." }).optional()
});
z$2.object({
	redirect_uris: z$2.array(z$2.string()),
	token_endpoint_auth_method: z$2.enum([
		"none",
		"client_secret_basic",
		"client_secret_post"
	...
	response_types: z$2.array(z$2.enum(["code", "token"])).default(["code"]).optional(),
	client_name: z$2.string().optional(),
	client_uri: z$2.string().optional(),
	logo_uri: z$2.string().optional(),
	scope: z$2.string().optional(),
	contacts: z$2.array(z$2.string()).optional(),
	tos_uri: z$2.string().optional(),
	policy_uri: z$2.string().optional(),
	jwks_uri: z$2.string().optional(),
	jwks: z$2.record(z$2.string(), z$2.any()).optional(),
	metadata: z$2.record(z$2.any(), z$2.any()).optional(),
	software_id: z$2.string().optional(),
	software_version: z$2.string().optional(),
	software_statement: z$2.string().optional()
});
z$2.record(z$2.any(), z$2.any());
z$2.object({ sessionToken: z$2.string().meta({ description: "The session token to set as active" }) });
z$2.object({ sessionToken: z$2.string().meta({ description: "The session token to revoke" }) });
z$2.object({
	callbackURL: z$2.string().meta({ description: "The URL to redirect to after the proxy" }),
	profile: z$2.string().optional().meta({ description: "Encrypted OAuth profile data" })
});
z$2.object({
	code: z$2.string().optional(),
	error: z$2.string().optional()
});
z$2.object({ idToken: z$2.string().meta({ description: "Google ID token, which the client obtains from the One Tap API" }) });
z$2.object({ token: z$2.string().meta({ description: "The token to verify. Eg: \"some-token\"" }) });
//#endregion
//#region node_modules/better-auth/dist/plugins/open-api/generator.mjs
...
function getTypeFromZodType(zodType) {
	if (zodType instanceof z$2.ZodDefault) return getTypeFromZodType(zodType.unwrap());
...
function getParameters(options) {
	...
	if (options.query instanceof z$2.ZodObject) Object.entries(options.query.shape).forEach(([key, value]) => {
		if (value instanceof z$2.ZodType) parameters.push({
			name: key,
			in: "query",
			schema: {
...
function getRequestBody(options) {
	...
	if (options.body instanceof z$2.ZodObject || options.body instanceof z$2.ZodOptional) {
		const shape = options.body.shape;
		if (!shape) return void 0;
		const properties = {};
		const required = [];
		Object.entries(shape).forEach(([key, value]) => {
			if (value instanceof z$2.ZodType) {
				properties[key] = processZodType(value);
				if (!(value instanceof z$2.ZodOptional)) required.push(key);
			}
		});
		return {
			required: options.body instanceof z$2.ZodOptional ? false : options.body ? true : false,
			content: { "application/json": { schema: {
				type: "object",
				properties,
				required
...
function processZodType(zodType) {
	if (zodType instanceof z$2.ZodOptional) {
	...
	if (zodType instanceof z$2.ZodDefault) {
		const innerSchema = processZodType(zodType.unwrap());
	...
	if (zodType instanceof z$2.ZodObject) {
...
//#endregion
//#region node_modules/better-auth/dist/plugins/open-api/logo.mjs
...
`;
//#endregion
//#region node_modules/better-auth/dist/plugins/open-api/index.mjs
	...
	return {
		...
		endpoints: {
			...
			openAPIReference: createAuthEndpoint(path, {
				...
				if (options?.disableDefaultReference) throw new APIError$2("NOT_FOUND");
				const schema = await generator(ctx.context, ctx.context.options);
...
z$2.object({
	organizationId: z$2.string().optional().meta({ description: "The id of the organization to create the role in. If not provided, the user's active organization will be used." }),
	role: z$2.string().meta({ description: "The name of the role to create" }),
	permission: z$2.record(z$2.string(), z$2.array(z$2.string())).meta({ description: "The permission to assign to the role" })
});
z$2.object({ organizationId: z$2.string().optional().meta({ description: "The id of the organization to create the role in. If not provided, the user's active organization will be used." }) }).and(z$2.union([z$2.object({ roleName: z$2.string().nonempty().meta({ description: "The name of the role to delete" }) }), z$2.object({ roleId: z$2.string().nonempty().meta({ description: "The id of the role to delete" }) })]));
z$2.object({ organizationId: z$2.string().optional().meta({ description: "The id of the organization to list roles for. If not provided, the user's active organization will be used." }) }).optional();
z$2.object({ organizationId: z$2.string().optional().meta({ description: "The id of the organization to read a role for. If not provided, the user's active organization will be used." }) }).and(z$2.union([z$2.object({ roleName: z$2.string().nonempty().meta({ description: "The name of the role to read" }) }), z$2.object({ roleId: z$2.string().nonempty().meta({ description: "The id of the role to read" }) })])).optional();
z$2.union([z$2.object({ roleName: z$2.string().nonempty().meta({ description: "The name of the role to update" }) }), z$2.object({ roleId: z$2.string().nonempty().meta({ description: "The id of the role to update" }) })]);
z$2.object({
	email: z$2.string().meta({ description: "The email address of the user to invite" }),
	role: z$2.union([z$2.string().meta({ description: "The role to assign to the user" }), z$2.array(z$2.string().meta({ description: "The roles to assign to the user" }))]).meta({ description: "The role(s) to assign to the user. It can be `admin`, `member`, owner. Eg: \"member\"" }),
	organizationId: z$2.string().meta({ description: "The organization ID to invite the user to" }).optional(),
	resend: z$2.boolean().meta({ description: "Resend the invitation email, if the user is already invited. Eg: true" }).optional(),
	teamId: z$2.union([z$2.string().meta({ description: "The team ID to invite the user to" }).optional(), z$2.array(z$2.string()).meta({ description: "The team IDs to invite the user to" }).optional()])
});
z$2.object({ invitationId: z$2.string().meta({ description: "The ID of the invitation to accept" }) });
z$2.object({ invitationId: z$2.string().meta({ description: "The ID of the invitation to reject" }) });
z$2.object({ invitationId: z$2.string().meta({ description: "The ID of the invitation to cancel" }) });
z$2.object({ id: z$2.string().meta({ description: "The ID of the invitation to get" }) });
z$2.object({ organizationId: z$2.string().meta({ description: "The ID of the organization to list invitations for" }).optional() }).optional();
z$2.object({
	userId: z$2.coerce.string().meta({ description: "The user Id which represents the user to be added as a member. If `null` is provided, then it's expected to provide session headers. Eg: \"user-id\"" }),
	role: z$2.union([z$2.string(), z$2.array(z$2.string())]).meta({ description: "The role(s) to assign to the new member. Eg: [\"admin\", \"sale\"]" }),
	organizationId: z$2.string().meta({ description: "An optional organization ID to pass. If not provided, will default to the user's active organization. Eg: \"org-id\"" }).optional(),
	teamId: z$2.string().meta({ description: "An optional team ID to add the member to. Eg: \"team-id\"" }).optional()
});
z$2.object({
	memberIdOrEmail: z$2.string().meta({ description: "The ID or email of the member to remove" }),
	organizationId: z$2.string().meta({ description: "The ID of the organization to remove the member from. If not provided, the active organization will be used. Eg: \"org-id\"" }).optional()
});
z$2.object({
	role: z$2.union([z$2.string(), z$2.array(z$2.string())]).meta({ description: "The new role to be applied. This can be a string or array of strings representing the roles. Eg: [\"admin\", \"sale\"]" }),
	memberId: z$2.string().meta({ description: "The member id to apply the role update to. Eg: \"member-id\"" }),
	organizationId: z$2.string().meta({ description: "An optional organization ID which the member is a part of to apply the role update. If not provided, you must provide session headers to get the active organization. Eg: \"organization-id\"" }).optional()
});
z$2.object({ organizationId: z$2.string().meta({ description: "The organization Id for the member to leave. Eg: \"organization-id\"" }) });
z$2.object({
	userId: z$2.string().meta({ description: "The user ID to get the role for. If not provided, will default to the current user's" }).optional(),
	organizationId: z$2.string().meta({ description: "The organization ID to list members for. If not provided, will default to the user's active organization. Eg: \"organization-id\"" }).optional(),
	organizationSlug: z$2.string().meta({ description: "The organization slug to list members for. If not provided, will default to the user's active organization. Eg: \"organization-slug\"" }).optional()
}).optional();
z$2.object({
	name: z$2.string().min(1).meta({ description: "The name of the organization" }),
	slug: z$2.string().min(1).meta({ description: "The slug of the organization" }),
	userId: z$2.coerce.string().meta({ description: "The user id of the organization creator. If not provided, the current user will be used. Should only be used by admins or when called by the server. server-only. Eg: \"user-id\"" }).optional(),
	logo: z$2.string().meta({ description: "The logo of the organization" }).optional(),
	metadata: z$2.record(z$2.string(), z$2.any()).meta({ description: "The metadata of the organization" }).optional(),
	keepCurrentActiveOrganization: z$2.boolean().meta({ description: "Whether to keep the current active organization active after creating a new one. Eg: true" }).optional()
});
z$2.object({ slug: z$2.string().meta({ description: "The organization slug to check. Eg: \"my-org\"" }) });
z$2.object({
	name: z$2.string().min(1).meta({ description: "The name of the organization" }).optional(),
	slug: z$2.string().min(1).meta({ description: "The slug of the organization" }).optional(),
	logo: z$2.string().meta({ description: "The logo of the organization" }).optional(),
	metadata: z$2.record(z$2.string(), z$2.any()).meta({ description: "The metadata of the organization" }).optional()
});
z$2.object({ organizationId: z$2.string().meta({ description: "The organization id to delete" }) });
z$2.optional(z$2.object({
	organizationId: z$2.string().meta({ description: "The organization id to get" }).optional(),
	organizationSlug: z$2.string().meta({ description: "The organization slug to get" }).optional(),
	membersLimit: z$2.number().or(z$2.string().transform((val) => parseInt(val))).meta({ description: "The limit of members to get. By default, it uses the membershipLimit option." }).optional()
}));
z$2.object({
	organizationId: z$2.string().meta({ description: "The organization id to set as active. It can be null to unset the active organization. Eg: \"org-id\"" }).nullable().optional(),
	organizationSlug: z$2.string().meta({ description: "The organization slug to set as active. It can be null to unset the active organization if organizationId is not provided. Eg: \"org-slug\"" }).optional()
});
//#endregion
//#region node_modules/better-auth/dist/plugins/organization/schema.mjs
var roleSchema = z$2.string();
var invitationStatus = z$2.enum([
	"pending",
	"accepted",
	"rejected",
	"canceled"
...
z$2.object({
	id: z$2.string().default(generateId),
	name: z$2.string(),
	slug: z$2.string(),
	logo: z$2.string().nullish().optional(),
	metadata: z$2.record(z$2.string(), z$2.unknown()).or(z$2.string().transform((v) => JSON.parse(v))).optional(),
	createdAt: z$2.date()
});
z$2.object({
	id: z$2.string().default(generateId),
	organizationId: z$2.string(),
	userId: z$2.coerce.string(),
	role: roleSchema,
	createdAt: z$2.date().default(() => /* @__PURE__ */ new Date())
});
z$2.object({
	id: z$2.string().default(generateId),
	organizationId: z$2.string(),
	...
	teamId: z$2.string().nullish(),
	inviterId: z$2.string(),
	expiresAt: z$2.date(),
	createdAt: z$2.date().default(() => /* @__PURE__ */ new Date())
});
z$2.object({
	id: z$2.string().default(generateId),
	name: z$2.string().min(1),
	organizationId: z$2.string(),
	createdAt: z$2.date(),
	updatedAt: z$2.date().optional()
});
z$2.object({
	id: z$2.string().default(generateId),
	teamId: z$2.string(),
...
z$2.union([z$2.enum(defaultRoles), z$2.array(z$2.enum(defaultRoles))]);
z$2.object({
	name: z$2.string().meta({ description: "The name of the team. Eg: \"my-team\"" }),
	organizationId: z$2.string().meta({ description: "The organization ID which the team will be created in. Defaults to the active organization. Eg: \"organization-id\"" }).optional()
});
z$2.object({
	teamId: z$2.string().meta({ description: `The team ID of the team to remove. Eg: "team-id"` }),
	organizationId: z$2.string().meta({ description: `The organization ID which the team falls under. If not provided, it will default to the user's active organization. Eg: "organization-id"` }).optional()
});
z$2.optional(z$2.object({ organizationId: z$2.string().meta({ description: `The organization ID which the teams are under to list. Defaults to the users active organization. Eg: "organization-id"` }).optional() }));
z$2.object({ teamId: z$2.string().meta({ description: "The team id to set as active. It can be null to unset the active team" }).nullable().optional() });
z$2.optional(z$2.object({ teamId: z$2.string().optional().meta({ description: "The team whose members we should return. If this is not provided the members of the current active team get returned." }) }));
z$2.object({
	teamId: z$2.string().meta({ description: "The team the user should be a member of." }),
	userId: z$2.coerce.string().meta({ description: "The user Id which represents the user to be added as a member." })
});
z$2.object({
	teamId: z$2.string().meta({ description: "The team the user should be removed from." }),
	userId: z$2.coerce.string().meta({ description: "The user which should be removed from the team." })
});
z$2.object({ organizationId: z$2.string().optional() }).and(z$2.union([z$2.object({
	permission: z$2.record(z$2.string(), z$2.array(z$2.string())),
	permissions: z$2.undefined()
}), z$2.object({
	permission: z$2.undefined(),
...
z$2.object({
	phoneNumber: z$2.string().meta({ description: "Phone number to sign in. Eg: \"+1234567890\"" }),
	password: z$2.string().meta({ description: "Password to use for sign in." }),
	rememberMe: z$2.boolean().meta({ description: "Remember the session. Eg: true" }).optional()
});
z$2.object({ phoneNumber: z$2.string().meta({ description: "Phone number to send OTP. Eg: \"+1234567890\"" }) });
z$2.object({
	phoneNumber: z$2.string().meta({ description: "Phone number to verify. Eg: \"+1234567890\"" }),
	code: z$2.string().meta({ description: "OTP code. Eg: \"123456\"" }),
	disableSession: z$2.boolean().meta({ description: "Disable session creation after verification. Eg: false" }).optional(),
	updatePhoneNumber: z$2.boolean().meta({ description: "Check if there is a session and update the phone number. Eg: true" }).optional()
}).and(z$2.record(z$2.string(), z$2.any()));
z$2.object({ phoneNumber: z$2.string() });
z$2.object({
	otp: z$2.string().meta({ description: "The one time password to reset the password. Eg: \"123456\"" }),
	phoneNumber: z$2.string().meta({ description: "The phone number to the account which intends to reset the password for. Eg: \"+1234567890\"" }),
	newPassword: z$2.string().meta({ description: `The new password. Eg: "new-and-secure-password"` })
});
z$2.object({
	walletAddress: z$2.string().regex(/^0[xX][a-fA-F0-9]{40}$/i).length(42),
	chainId: z$2.number().int().positive().optional().default(1)
});
//#endregion
//#region node_modules/better-auth/dist/plugins/two-factor/constant.mjs
var TWO_FACTOR_COOKIE_NAME = "two_factor";
var TRUST_DEVICE_COOKIE_NAME = "trust_device";
//#endregion
//#region node_modules/better-auth/dist/plugins/two-factor/verify-two-factor.mjs
...
//#endregion
//#region node_modules/better-auth/dist/plugins/two-factor/backup-codes/index.mjs
...
var verifyBackupCodeBodySchema = z$2.object({
	code: z$2.string().meta({ description: `A backup code to verify. Eg: "123456"` }),
	disableSession: z$2.boolean().meta({ description: "If true, the session cookie will not be set." }).optional(),
	trustDevice: z$2.boolean().meta({ description: "If true, the device will be trusted for 30 days. It'll be refreshed on every sign in request within this time. Eg: true" }).optional()
});
var viewBackupCodesBodySchema = z$2.object({ userId: z$2.coerce.string().meta({ description: `The user ID to view all backup codes. Eg: "user-id"` }) });
var generateBackupCodesBodySchema = z$2.object({ password: z$2.string().meta({ description: "The users password." }) });
...
//#endregion
//#region node_modules/better-auth/dist/plugins/two-factor/utils.mjs
var defaultKeyHasher = async (token) => {
	const hash = await createHash("SHA-256").digest(new TextEncoder().encode(token));
	return base64Url.encode(new Uint8Array(hash), { padding: false });
};
//#endregion
//#region node_modules/better-auth/dist/plugins/two-factor/otp/index.mjs
var verifyOTPBodySchema = z$2.object({
	code: z$2.string().meta({ description: "The otp code to verify. Eg: \"012345\"" }),
	trustDevice: z$2.boolean().optional().meta({ description: "If true, the device will be trusted for 30 days. It'll be refreshed on every sign in request within this time. Eg: true" })
});
var send2FaOTPBodySchema = z$2.object({ trustDevice: z$2.boolean().optional().meta({ description: "If true, the device will be trusted for 30 days. It'll be refreshed on every sign in request within this time. Eg: true" }) }).optional();
/**
...
//#endregion
//#region node_modules/better-auth/dist/plugins/two-factor/schema.mjs
var schema = {
...
//#endregion
//#region node_modules/better-auth/dist/plugins/two-factor/totp/index.mjs
var generateTOTPBodySchema = z$2.object({ secret: z$2.string().meta({ description: "The secret to generate the TOTP code" }) });
var getTOTPURIBodySchema = z$2.object({ password: z$2.string().meta({ description: "User password" }) });
var verifyTOTPBodySchema = z$2.object({
	code: z$2.string().meta({ description: "The otp code to verify. Eg: \"012345\"" }),
	trustDevice: z$2.boolean().meta({ description: "If true, the device will be trusted for 30 days. It'll be refreshed on every sign in request within this time. Eg: true" }).optional()
});
var totp2fa = (options) => {
...
//#endregion
//#region node_modules/better-auth/dist/plugins/two-factor/index.mjs
var enableTwoFactorBodySchema = z$2.object({
	password: z$2.string().meta({ description: "User password" }),
	issuer: z$2.string().meta({ description: "Custom issuer for the TOTP URI" }).optional()
});
var disableTwoFactorBodySchema = z$2.object({ password: z$2.string().meta({ description: "User password" }) });
...
var signInUsernameBodySchema = z$2.object({
	username: z$2.string().meta({ description: "The username of the user" }),
	password: z$2.string().meta({ description: "The password of the user" }),
	rememberMe: z$2.boolean().meta({ description: "Remember the user session" }).optional(),
	callbackURL: z$2.string().meta({ description: "The URL to redirect to after email verification" }).optional()
});
var isUsernameAvailableBodySchema = z$2.object({ username: z$2.string().meta({ description: "The username to check" }) });
...
var env$1 = createEnv({
	...
	server: {
		...
		OAUTH_SCOPES: z.string().min(1).transform((value) => value.split(" ")).default([
			"openid",
			"profile",
			"email"
		]),
...
//#endregion
//#region src/integrations/drizzle/schema.ts
...
var user = pg.pgTable("user", {
	id: pg.uuid("id").notNull().primaryKey().$defaultFn(() => generateId$1()),
	image: pg.text("image"),
...
var verification = pg.pgTable("verification", {
	id: pg.uuid("id").notNull().primaryKey().$defaultFn(() => generateId$1()),
...
var twoFactor = pg.pgTable("two_factor", {
	id: pg.uuid("id").notNull().primaryKey().$defaultFn(() => generateId$1()),
	userId: pg.uuid("user_id").notNull().references(() => user.id, { onDelete: "cascade" }),
...
var passkey = pg.pgTable("passkey", {
	id: pg.uuid("id").notNull().primaryKey().$defaultFn(() => generateId$1()),
	name: pg.text("name"),
...
var resume = pg.pgTable("resume", {
	...
	password: pg.text("password"),
	data: pg.jsonb("data").notNull().$type().$defaultFn(() => defaultResumeData),
	userId: pg.uuid("user_id").notNull().references(() => user.id, { onDelete: "cascade" }),
	createdAt: pg.timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
	updatedAt: pg.timestamp("updated_at", { withTimezone: true }).notNull().defaultNow().$onUpdate(() => /* @__PURE__ */ new Date())
}, (t) => [
...
var resumeStatistics = pg.pgTable("resume_statistics", {
	id: pg.uuid("id").notNull().primaryKey().$defaultFn(() => generateId$1()),
	...
	resumeId: pg.uuid("resume_id").unique().notNull().references(() => resume.id, { onDelete: "cascade" }),
	createdAt: pg.timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
	updatedAt: pg.timestamp("updated_at", { withTimezone: true }).notNull().defaultNow().$onUpdate(() => /* @__PURE__ */ new Date())
});
var apikey = pg.pgTable("apikey", {
	id: pg.uuid("id").notNull().primaryKey().$defaultFn(() => generateId$1()),
	name: pg.text("name"),
	start: pg.text("start"),
	...
	referenceId: pg.uuid("reference_id").notNull().references(() => user.id, { onDelete: "cascade" }),
	...
	rateLimitTimeWindow: pg.integer("rate_limit_time_window").default(864e5),
...
//#endregion
//#region src/integrations/drizzle/client.ts
function getPool() {
...
//#endregion
//#region src/integrations/email/service.ts
var isSmtpEnabled = () => {
...
//#endregion
//#region src/integrations/auth/config.ts
...
//#endregion
//#region src/integrations/orpc/context.ts
...
var protectedProcedure = publicProcedure.use(async ({ context, next }) => {
	if (!context.user) throw new ORPCError$1("UNAUTHORIZED");
	return next({ context: {
		...context,
		user: context.user
	} });
...
var serverOnlyProcedure = publicProcedure.use(async ({ context, next }) => {
	const headers = context.reqHeaders ?? new Headers();
	if (!(env$1.FLAG_DEBUG_PRINTER, headers.get("x-server-side-call") === "true")) throw new ORPCError$1("UNAUTHORIZED", { message: "This endpoint can only be called from server-side code" });
	return next({ context });
});
//#endregion
//#region src/integrations/ai/prompts/chat-system.md?raw
...
//#endregion
//#region src/integrations/ai/prompts/docx-parser-system.md?raw
...
//#endregion
//#region src/integrations/ai/prompts/docx-parser-user.md?raw
...
//#endregion
//#region src/integrations/ai/prompts/pdf-parser-system.md?raw
...
//#endregion
//#region src/integrations/ai/prompts/pdf-parser-user.md?raw
var pdf_parser_user_default = "The resume PDF is attached as a file in this message.\r\n\r\nProcess the attached document and return the final JSON object only, strictly following the system rules and the provided schema template.";
//#endregion
//#region src/utils/resume/patch.ts
/**
...
*/
var jsonPatchOperationSchema = z$1.discriminatedUnion("op", [
	z$1.object({
		op: z$1.literal("add"),
		path: z$1.string(),
		value: z$1.any()
...
function applyResumePatches(data, operations) {
	...
	let patched;
	try {
		patched = jsonpatch.applyPatch(data, operations, false, false).newDocument;
	} catch (error) {
...
//#endregion
//#region src/integrations/ai/tools/patch-resume.ts
var patchResumeInputSchema = z$1.object({ operations: z$1.array(jsonPatchOperationSchema).min(1).describe("Array of JSON Patch (RFC 6902) operations to apply to the resume") });
...
//#endregion
//#region src/integrations/orpc/services/ai.ts
...
function getModel(input) {
	...
	return match(provider).with("openai", () => createOpenAI({
		apiKey,
		baseURL
		...
		apiKey,
		baseURL
	}).languageModel(model)).with("vercel-ai-gateway", () => createGateway({
		apiKey,
		baseURL
...
var aiCredentialsSchema = z$1.object({
	provider: aiProviderSchema,
	model: z$1.string(),
	apiKey: z$1.string(),
	baseURL: z$1.string()
});
var fileInputSchema = z$1.object({
	name: z$1.string(),
	data: z$1.string().max(MAX_AI_FILE_BASE64_CHARS, "File is too large. Maximum size is 10MB.")
});
...
async function chat(input) {
	return streamToEventIterator(streamText({
		model: getModel(input),
...
//#endregion
//#region src/integrations/orpc/router/ai.ts
var aiRouter = {
	...
	parseDocx: protectedProcedure.route({
		...
		...aiCredentialsSchema.shape,
		file: fileInputSchema,
		mediaType: z$1.enum(["application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"])
	})).errors({
		BAD_GATEWAY: {
			message: "The AI provider returned an error or is unreachable.",
			status: 502
...
//#endregion
//#region src/integrations/orpc/services/storage.ts
...
//#endregion
//#region src/integrations/orpc/router/auth.ts
var authRouter = {
...
var flagsRouter = { get: publicProcedure.route({
	...
	disableSignups: z$1.boolean().describe("Whether new user signups are disabled on this instance."),
	disableEmailAuth: z$1.boolean().describe("Whether email-based authentication is disabled on this instance.")
})).handler(() => flagsService.getFlags()) };
//#endregion
//#region src/schema/page.ts
...
//#endregion
//#region src/utils/printer-token.ts
...
var generatePrinterToken = (resumeId) => {
	...
	return `${payloadBase64}.${createHash$1("sha256").update(`${payloadBase64}.${env$1.AUTH_SECRET}`).digest("hex")}`;
};
/**
...
var verifyPrinterToken = (token) => {
	...
	const expectedSignature = createHash$1("sha256").update(`${payloadBase64}.${env$1.AUTH_SECRET}`).digest("hex");
...
//#endregion
//#region src/integrations/orpc/services/printer.ts
...
var printerService = {
	...
	printResumeAsPDF: async (input) => {
		...
		try {
			...
			throw new ORPCError$1("INTERNAL_SERVER_ERROR", error);
		} finally {
...
//#endregion
//#region src/integrations/orpc/helpers/resume-access.ts
...
var signResumeAccessToken = (resumeId, passwordHash) => createHash$1("sha256").update(`${resumeId}:${passwordHash}`).digest("hex");
...
var resumeService = {
	...
	statistics: {
		...
		increment: async (input) => {
			...
			const lastViewedAt = input.views ? sql$1`now()` : void 0;
			const lastDownloadedAt = input.downloads ? sql$1`now()` : void 0;
			await db.insert(resumeStatistics).values({
				...
				set: {
					views: sql$1`${resumeStatistics.views} + ${views}`,
					downloads: sql$1`${resumeStatistics.downloads} + ${downloads}`,
					lastViewedAt,
					lastDownloadedAt
				}
			});
	...
	list: async (input) => {
		...
		}).from(resume).where(and(eq(resume.userId, input.userId), match(input.tags.length).with(0, () => void 0).otherwise(() => arrayContains(resume.tags, input.tags)))).orderBy(match(input.sort).with("lastUpdatedAt", () => desc(resume.updatedAt)).with("createdAt", () => asc(resume.createdAt)).with("name", () => asc(resume.name)).exhaustive());
	},
	getById: async (input) => {
		const [resume$4] = await db.select({
			...
			isPublic: resume.isPublic,
			isLocked: resume.isLocked,
			hasPassword: sql$1`${resume.password} IS NOT NULL`
...
//#endregion
//#region src/integrations/orpc/router/printer.ts
var printerRouter = {
	...
	}).input(z$1.object({ id: z$1.string().describe("The unique identifier of the resume to export.") })).output(z$1.object({ url: z$1.string().describe("The URL to download the generated PDF file.") })).handler(async ({ input, context }) => {
	...
	}).input(z$1.object({ id: z$1.string().describe("The unique identifier of the resume.") })).output(z$1.object({ url: z$1.string().nullable().describe("The URL to the screenshot image, or null.") })).handler(async ({ context, input }) => {
		try {
...
//#endregion
//#region src/schema/resume/sample.ts
...
//#endregion
//#region src/integrations/orpc/dto/resume.ts
var resumeSchema = createSelectSchema(resume, {
	id: z$1.string().describe("The ID of the resume."),
	name: z$1.string().min(1).describe("The name of the resume."),
	slug: z$1.string().min(1).describe("The slug of the resume."),
	tags: z$1.array(z$1.string()).describe("The tags of the resume."),
	isPublic: z$1.boolean().describe("Whether the resume is public."),
	isLocked: z$1.boolean().describe("Whether the resume is locked."),
	password: z$1.string().min(6).nullable().describe("The password of the resume, if any."),
	data: resumeDataSchema.loose(),
	userId: z$1.string().describe("The ID of the user who owns the resume."),
	createdAt: z$1.date().describe("The date and time the resume was created."),
	updatedAt: z$1.date().describe("The date and time the resume was last updated.")
});
var resumeDto = {
	list: {
		input: z$1.object({
			tags: z$1.array(z$1.string()).optional().default([]),
			sort: z$1.enum([
				"lastUpdatedAt",
				"createdAt",
				"name"
		...
		output: z$1.array(resumeSchema.omit({
			data: true,
			password: true,
			userId: true
		}))
	...
	getById: {
		...
		output: resumeSchema.omit({
			...
			createdAt: true,
			updatedAt: true
		}).extend({ hasPassword: z$1.boolean() })
	},
	getBySlug: {
		input: z$1.object({
			username: z$1.string(),
	...
	create: {
		input: resumeSchema.pick({
			...
			slug: true,
			tags: true
		}).extend({ withSampleData: z$1.boolean().default(false) }),
		output: z$1.string().describe("The ID of the created resume.")
	},
	import: {
		input: resumeSchema.pick({ data: true }),
		output: z$1.string().describe("The ID of the imported resume.")
	},
	update: {
		input: resumeSchema.pick({
			name: true,
	...
	setPassword: {
		input: resumeSchema.pick({ id: true }).extend({ password: z$1.string().min(6).max(64) }),
		output: z$1.void()
	},
	removePassword: {
		input: resumeSchema.pick({ id: true }),
	...
	patch: {
		input: z$1.object({
			id: z$1.string().describe("The ID of the resume to patch."),
			operations: z$1.array(jsonPatchOperationSchema).min(1).describe("An array of JSON Patch (RFC 6902) operations to apply to the resume data.")
		}),
		output: resumeSchema.omit({
			password: true,
			userId: true,
	...
	duplicate: {
		...
		output: z$1.string().describe("The ID of the duplicated resume.")
	},
	delete: {
		input: resumeSchema.pick({ id: true }),
		output: z$1.void()
...
var resumeRouter$1 = {
	...
	}).output(z$1.array(z$1.string())).handler(async ({ context }) => {
	...
	statistics: {
		...
		}).input(z$1.object({ id: z$1.string().describe("The unique identifier of the resume.") })).output(z$1.object({
			isPublic: z$1.boolean().describe("Whether the resume is currently public."),
			views: z$1.number().describe("Total number of times the resume has been viewed."),
			downloads: z$1.number().describe("Total number of times the resume has been downloaded."),
			lastViewedAt: z$1.date().nullable().describe("Timestamp of the last view, or null if never viewed."),
			lastDownloadedAt: z$1.date().nullable().describe("Timestamp of the last download, or null if never downloaded.")
		...
		increment: publicProcedure.route({
			...
			id: z$1.string(),
			views: z$1.boolean().default(false),
			downloads: z$1.boolean().default(false)
		})).handler(async ({ input }) => {
	...
	verifyPassword: publicProcedure.route({
		...
		username: z$1.string().min(1).describe("The username of the resume owner."),
		slug: z$1.string().min(1).describe("The slug of the resume."),
		password: z$1.string().min(1).describe("The password to verify.")
	})).output(z$1.boolean()).handler(async ({ input }) => {
...
//#endregion
//#region src/integrations/orpc/services/statistics.ts
...
var statisticsRouter = {
	...
	}).output(z$1.number().describe("The total number of registered users.")).handler(async () => {
	...
	}).output(z$1.number().describe("The total number of resumes created.")).handler(async () => {
		return await statisticsService.resume.getCount();
	}) }
};
//#endregion
//#region src/integrations/orpc/router/storage.ts
var storageService = getStorageService();
var fileSchema = z$1.file().max(10 * 1024 * 1024, "File size must be less than 10MB");
var filenameSchema = z$1.object({ filename: z$1.string().min(1).describe("The path or filename of the file to delete.") });
...
//#endregion
//#region src/integrations/orpc/router/index.ts
var router_default = {
	ai: aiRouter,
	auth: authRouter,
	flags: flagsRouter,
	...
	storage: {
		...
		}).input(fileSchema).output(z$1.object({
			url: z$1.string().describe("The public URL to access the uploaded file."),
			path: z$1.string().describe("The storage path of the uploaded file."),
			contentType: z$1.string().describe("The MIME type of the uploaded file.")
		...
		deleteFile: protectedProcedure.route({
		...
		}).input(filenameSchema).output(z$1.void()).errors({
			NOT_FOUND: {
			...
			if (isUnsafeStorageKey(key) || !key.startsWith(userPrefix)) throw new ORPCError$1("FORBIDDEN");
			if (!await storageService.delete(key)) throw new ORPCError$1("NOT_FOUND");
		})
	},
...
//#endregion
//#region src/integrations/orpc/client.ts
var getORPCClient = () => {
...
var orpc = createTanstackQueryUtils(client);
//#endregion
export { db as _, printerService as a, pageDimensionsAsPixels as c, sanitizeCss as d, sanitizeHtml as f, logger$1 as g, auth as h, router_default as i, getStorageService as l, jsonPatchOperationSchema as m, getORPCClient as n, verifyPrinterToken as o, applyResumePatches as p, orpc as r, pageDimensionsAsMillimeters as s, client as t, inferContentType as u, env$1 as v };
