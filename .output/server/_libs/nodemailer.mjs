...
var require_errors = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	...
	module.exports = Object.keys(ERROR_CODES).reduce((exports$1, code) => {
		exports$1[code] = code;
		return exports$1;
	}, { ERROR_CODES });
}));
...
var require_shared = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var formatDNSValue = (value, extra) => {
		...
		let host = null;
		if (addresses.length === 1) host = addresses[0];
		else if (addresses.length > 1) host = addresses[Math.floor(Math.random() * addresses.length)];
		...
		_getStream(content) {
			...
			if (content._resolvedValue) {
				...
				if (this.disableFileAccess) {
					contentStream = new PassThrough$2();
					setImmediate(() => {
						let err = /* @__PURE__ */ new Error("File access rejected for " + content.path);
				...
				if (this.disableUrlAccess) {
					contentStream = new PassThrough$2();
					setImmediate(() => {
						let err = /* @__PURE__ */ new Error("Url access rejected for " + content.href);
			...
			let onSocketData = (chunk) => {
				...
				if (match = headers.match(/\r\n\r\n/)) {
					...
					if (!match || (match[1] || "").charAt(0) !== "2") {
						...
						let err = /* @__PURE__ */ new Error("Invalid response from proxy" + (match && ": " + match[1] || ""));
		...
		constructor(options, logger) {
			...
			if (options && options.serviceClient) {
				if (!options.privateKey || !options.user) {
					let err = /* @__PURE__ */ new Error("Options \"privateKey\" and \"user\" are required for service account!");
		...
		*/
		getToken(renew, callback) {
			...
			if (!this.provisionCallback && !this.options.refreshToken && !this.options.serviceClient) {
				...
				let err = /* @__PURE__ */ new Error("Can't create new access token for user");
				err.code = errors.EOAUTH2;
				return callback(err);
			}
		...
		generateToken(callback) {
			...
			if (this.options.serviceClient) {
				...
				try {
					...
					let err = /* @__PURE__ */ new Error("Can't generate token. Check your auth options");
					err.code = errors.EOAUTH2;
					return callback(err);
				}
				urlOptions = {
			...
			this.postRequest(this.options.accessUrl, urlOptions, this.options, (error, body) => {
				...
				if (!data || typeof data !== "object") {
					...
					let err = /* @__PURE__ */ new Error("Invalid authentication response");
					err.code = errors.EOAUTH2;
					return callback(err);
				}
				let logData = {};
				...
				let err = /* @__PURE__ */ new Error("No access token");
				err.code = errors.EOAUTH2;
				return callback(err);
			});
		}
		...
		*/
		send(mail, done) {
			...
			if ([].concat(envelope.from || []).concat(envelope.to || []).some((addr) => /^-/.test(addr))) {
				let err = /* @__PURE__ */ new Error("Can not send mail. Invalid envelope addresses.");
			...
			if (sendmail) {
				...
				let err = /* @__PURE__ */ new Error("sendmail was not found");
				err.code = errors.ESENDMAIL;
				return callback(err);
			}
		}
