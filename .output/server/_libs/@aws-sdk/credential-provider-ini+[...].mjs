...
import { At as HttpRequest, F as readFile, N as parseKnownFiles, R as getProfileName, ct as chain, j as init_dist_es$1, kt as init_dist_es$2, ot as require_client, st as init_dist_es, ut as CredentialsProviderError } from "./client-s3+[...].mjs";
...
var resolveCredentialSource = (credentialSource, profileName, logger) => {
	const sourceProvidersMap = {
		EcsContainer: async (options) => {
			...
			const { fromContainerMetadata } = await import("../_22.mjs");
			logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
			return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
		},
		Ec2InstanceMetadata: async (options) => {
			logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
			const { fromInstanceMetadata } = await import("../_22.mjs");
...
var resolveAssumeRoleCredentials = async (profileName, profiles, options, callerClientConfig, visitedProfiles = {}, resolveProfileData) => {
	...
	if (!options.roleAssumer) {
		const { getDefaultRoleAssumer } = await import("../_16.mjs").then((m) => /* @__PURE__ */ __toESM(m.default));
...
var LoginCredentialsFetcher = class LoginCredentialsFetcher {
	...
	async refresh(token) {
		const { SigninClient, CreateOAuth2TokenCommand } = await import("../_14.mjs").then((m) => /* @__PURE__ */ __toESM(m.default));
...
var resolveProcessCredentials = async (options, profile) => import("../_10.mjs").then(({ fromProcess }) => fromProcess({
	...options,
	profile
...
var resolveWebIdentityCredentials = async (profile, options, callerClientConfig) => import("../_13.mjs").then(({ fromTokenFile }) => fromTokenFile({
