...
function Ye(e) {
	...
	{
		let x = -1;
		for (const f of s) f.hasAttribute("data-panel") && (x++, f.ariaDisabled === null && (m++, u === -1 && (u = x), c = x));
	}
	if (m > 1) {
		let x = -1;
		for (const f of s) if (f.hasAttribute("data-panel")) {
			...
			if (h) {
				if (d) {
					...
					} else v.length ? z = v : z = [n === "horizontal" ? new DOMRect(g.right, y.top, y.left - g.right, y.height) : new DOMRect(y.left, g.bottom, y.width, y.top - g.bottom)];
					for (const S of z) {
...
function se({ delta: e, initialLayout: t, panelConstraints: n, pivotIndices: o, prevLayout: i, trigger: s }) {
	...
	switch (s) {
		...
		default: {
			...
			if (y && G(g, S) < 0) if (e > 0) {
				const p = S - z, P = p / 2;
				G(g + e, S) < 0 && (e = G(e, P) <= 0 ? 0 : p);
			} else {
				const p = S - z, P = 100 - p / 2;
				G(g - e, S) < 0 && (e = G(100 + e, P) > 0 ? 0 : -p);
			}
			break;
		}
	}
	{
		...
		for (;;) {
			...
			if (g += p, h += f, h < 0 || h >= n.length) break;
		}
	...
	}
	{
		...
		for (; h >= 0 && h < n.length;) {
			...
			if (!I(y, S) && (d += y - S, u[h] = S, d.toFixed(3).localeCompare(Math.abs(e).toFixed(3), void 0, { numeric: !0 }) >= 0)) break;
			e < 0 ? h-- : h++;
		}
	}
	if (Ot(a, u)) return i;
	{
		...
		if (u[f] = y, !I(y, g)) {
			...
			for (; p >= 0 && p < n.length;) {
				...
				if (I(P, C) || (z -= C - P, u[p] = C), I(z, 0)) break;
				e > 0 ? p-- : p++;
			}
		}
	}
	if (!I(Object.values(u).reduce((f, h) => h + f, 0), 100, .1)) return i;
...
function B({ layout: e, panelConstraints: t }) {
	...
	if (!I(s, 0)) for (let r = 0; r < t.length; r++) {
		...
		if (a !== c && (s -= c - a, o[r] = c, I(s, 0))) break;
	}
...
function et({ groupId: e, panelId: t }) {
	const n = () => {
		const r = U();
		for (const [a, { defaultLayoutDeferred: u, derivedPanelConstraints: c, layout: m, groupSize: d, separatorToPanels: v }] of r) if (a.id === e) return {
		...
		const a = s();
		if (r === a) return;
		const { defaultLayoutDeferred: u, derivedPanelConstraints: c, group: m, groupSize: d, layout: v, separatorToPanels: x } = n(), f = m.panels.findIndex((z) => z.id === t), h = f === m.panels.length - 1, y = B({
			layout: se({
				delta: h ? a - r : r - a,
				initialLayout: v,
				panelConstraints: c,
	...
	return {
		...
		resize: (r) => {
			const { group: a } = n(), { element: u } = i(), c = Q({ group: a });
			l(D(te({
				groupSize: c,
				panelElement: u,
				styleProp: r
...
function V(e, t) {
	const n = fe(e), o = $(n.id, !0), i = n.separators.find((m) => m.element === e);
	L(i, "Matching separator not found");
	const s = o.separatorToPanels.get(i);
	L(s, "Matching panels not found");
	const l = s.map((m) => n.panels.indexOf(m)), a = tt({ groupId: n.id }).getLayout(), c = B({
		layout: se({
			delta: t,
			initialLayout: a,
	...
	if (!n.disabled) switch (e.key) {
		...
		case "Enter": {
			...
			if (L(m, "Panel metadata not found"), m.collapsible) {
				const d = l[c.id];
				V(t, (m.collapsedSize === d ? o.mutableState.expandedPanelSizes[c.id] ?? m.minSize : m.collapsedSize) - d);
			}
			break;
		}
		case "F6": {
...
var Nt = (e) => e, he = () => {}, nt = 1, ot = 2, it = 4, rt = 8, Ge = 3, Ae = 12;
var ce;
function Ne() {
...
function st({ document: e, event: t, hitRegions: n, initialLayoutMap: o, mountedGroups: i, pointerDownAtPoint: s, prevCursorFlags: l }) {
	...
	n.forEach((u) => {
		...
		if (!h || !g) return;
		const { defaultLayoutDeferred: y, derivedPanelConstraints: z, groupSize: S, layout: p, separatorToPanels: P } = g;
		if (z && p && P) {
			const R = se({
				delta: f,
				initialLayout: h,
...
function Ht({ group: e, nextGroupSize: t, prevGroupSize: n, prevLayout: o }) {
	...
	if (r.forEach((m, d) => {
		c[d] = m;
	}), s > 0) for (const m of a) c[m] = D((o[m] ?? 0) / s * u);
	else {
		const m = D(u / a.length);
		for (const d of a) c[d] = m;
	}
	return c;
}
function Vt(e, t) {
...
function jt(e) {
		...
		for (const h of f) {
			...
			if (y === e.element) {
				if (t) {
					...
					const S = $(e.id);
					if (!S) return;
					const p = ge(e), P = S.defaultLayoutDeferred ? je(p) : S.layout, C = B({
						layout: Ht({
							group: e,
							nextGroupSize: z,
					...
					if (!S.defaultLayoutDeferred && j(S.layout, C) && $t(S.derivedPanelConstraints, p) && S.groupSize === z) return;
					_(e, {
	...
	const m = B({
		layout: e.mutableState.layouts[a] ?? u ?? je(r),
		panelConstraints: r
	...
	return Ye(e).forEach((f) => {
		...
		t = !1, q.set(d, Math.max(0, (q.get(d) ?? 0) - 1)), Lt(e), e.separators.forEach((h) => {
			h.element.removeEventListener("keydown", Te);
		}), q.get(d) || (d.removeEventListener("dblclick", De, !0), d.removeEventListener("pointerdown", Oe, !0), d.removeEventListener("pointerleave", Fe), d.removeEventListener("pointermove", $e), d.removeEventListener("pointerout", He), d.removeEventListener("pointerup", Ve, !0)), s.disconnect();
	};
}
function Bt() {
...
function Ut({ children: e, className: t, defaultLayout: n, disableCursor: o, disabled: i, elementRef: s, groupRef: l, id: r, onLayoutChange: a, onLayoutChanged: u, orientation: c = "horizontal", resizeTargetMinimumSize: m = {
	coarse: 20,
	fine: 10
}, style: d, ...v }) {
	...
	}), g = xe(r), y = (0, import_react.useRef)(null), [z, S] = Bt(), p = (0, import_react.useRef)({
	...
	const R = oe((b, w) => {
		...
		registerPanel: (b) => {
			const w = p.current;
			return w.panels = Se(c, [...w.panels, b]), S(), () => {
				w.panels = w.panels.filter((E) => E !== b), S();
			};
		},
		registerSeparator: (b) => {
			const w = p.current;
			return w.separators = Se(c, [...w.separators, b]), S(), () => {
...
function qt({ children: e, className: t, collapsedSize: n = "0%", collapsible: o = !1, defaultSize: i, disabled: s, elementRef: l, groupResizeBehavior: r = "preserve-relative-size", id: a, maxSize: u = "100%", minSize: c = "0%", onResize: m, panelRef: d, style: v, ...x }) {
	const f = !!a, h = xe(a), g = Pe({ disabled: s }), y = (0, import_react.useRef)(null), z = we(y, l), { getPanelStyles: S, id: p, orientation: P, registerPanel: R, togglePanelDisabled: C } = Le(), X = m !== null, F = oe((w, E, M) => {
		m?.(w, a, M);
	});
	K(() => {
		const w = y.current;
	...
	const b = (0, import_react.useSyncExternalStore)((w) => ye(p, w), () => JSON.stringify(S(p, h)), () => JSON.stringify(S(p, h)));
...
function Zt({ children: e, className: t, disabled: n, elementRef: o, id: i, style: s, ...l }) {
	const r = xe(i), a = Pe({ disabled: n }), [u, c] = (0, import_react.useState)({}), [m, d] = (0, import_react.useState)("inactive"), v = (0, import_react.useRef)(null), x = we(v, o), { disableCursor: f, id: h, orientation: g, registerSeparator: y, toggleSeparatorDisabled: z } = Le(), S = g === "horizontal" ? "vertical" : "horizontal";
	K(() => {
		const P = v.current;
		if (P !== null) {
			const R = {
	...
	let p;
	return n && !f && (p = "not-allowed"), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		...l,
...
//#endregion
export { qt as i, Zt as n, an as r, Ut as t };
