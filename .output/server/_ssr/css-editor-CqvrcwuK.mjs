...
import { n as Le, t as Ft } from "../_libs/monaco-editor__react.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/css-editor-CqvrcwuK.js
...
var CSS_CLASS_SELECTOR_PATTERN = /\.([\w-]*)$/;
function CSSMonacoEditor({ theme, defaultValue, onChange }) {
	const monaco = Le();
	(0, import_react.useEffect)(() => {
		...
		const completionProvider = monaco.languages.registerCompletionItemProvider("css", {
			...
			provideCompletionItems: (model, position) => {
				...
				}).match(CSS_CLASS_SELECTOR_PATTERN);
				...
				return { suggestions: CSS_SELECTORS.filter((selector) => selector.toLowerCase().startsWith(`.${prefix}`)).map((selector) => ({
					label: selector,
...
//#endregion
export { CSSMonacoEditor as default };
